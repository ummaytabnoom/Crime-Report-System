...
<%
    ...
    if (isPolice &&
        ...
        try {
            ...
            updatePs = updateConn.prepareStatement(
                    "UPDATE PERMANENT_REPORTS SET STATUS=? WHERE CRIME_ID=?");

            updatePs.setString(1, newStatus);
            updatePs.setInt(2, crimeId);
    ...
    try {
        ...
        while (crimesRs.next()) {
            ...
            crime.put("demoImg",
                    (demoBytes != null)
                    ? Base64.getEncoder().encodeToString(demoBytes)
                    : "");
...
<div class="navbar">
    ...
    <div id="dropdownMenu" class="dropdown">
        ...
        <% } %>
                <% if(isPolice){ %>
            <a href="PoliceHome.jsp">Police penel</a>
        <% } %>
...
<div class="crime-container">
    ...
    <% if(isAdmin || isPolice){ %>
        ...
        <!-- Only Admin & Police can click and see info -->
    ...
    <% } else { %>
        ...
        <!-- Public users only see Anonymous text -->
...
<% } else { %>
    ...
    <% if (demoImg != null && !demoImg.isEmpty()) { %>
        ...
        <img src="data:image/jpeg;base64,<%= demoImg %>"
             class="crime-image">
...
function filterCrimes(){
    ...
    const containers =
        document.getElementsByClassName("crime-container");

    for(let i=0;i<containers.length;i++){

        const text =
            containers[i].innerText.toLowerCase();
