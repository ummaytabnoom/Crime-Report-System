...
<%
    ...
    try{
        ...
        if("POST".equalsIgnoreCase(request.getMethod())){
            ...
            if(action != null && crimeIdParam != null){
                ...
                if("delete".equalsIgnoreCase(action)){
                    ...
                    // 1. Insert into PERMANENT_REPORTS
                    PreparedStatement insertStmt = conn.prepareStatement(

                        "INSERT INTO PERMANENT_REPORTS " +
                        "(ID, USER_NAME, FULL_NAME, PROFILE_PICTURE, " +
                        "ZILLA, UPAZILLA, POLICE_STATION, AREA, ROAD_NAME, ROAD_NO, " +
                        "DATE_OF_INCIDENT, CATEGORY, DESCRIPTION, STATUS, " +
                        "DEMO_PICTURE, HIDE_IDENTITY) " +

                        "SELECT r.ID, r.USER_NAME, r.FULL_NAME, u.PROFILE_PICTURE, " +
                        "r.ZILLA, r.UPAZILLA, r.POLICE_STATION, r.AREA, r.ROAD_NAME, r.ROAD_NO, " +
                        "r.DATE_OF_INCIDENT, r.CATEGORY, r.DESCRIPTION, 'ACCEPTED', " +
                        "r.DEMO_PICTURE, r.HIDE_IDENTITY " +

                        "FROM REPORTED_CRIMES r " +
                        "JOIN REGISTERED_USERS u ON r.ID = u.ID " +
                        "WHERE r.CRIME_ID = ?"
                    );

                    insertStmt.setInt(1, crimeId);
                    int inserted = insertStmt.executeUpdate();
                    insertStmt.close();

                    if(inserted > 0){

                        // 2. Instead of deleting → UPDATE STATUS
                        PreparedStatement updateStmt = conn.prepareStatement(
                            "UPDATE REPORTED_CRIMES SET STATUS='ACCEPTED' WHERE CRIME_ID=?"
                        );

                        updateStmt.setInt(1, crimeId);
        ...
        while(crimesRs.next()){
            ...
            Map<String,Object> crime =
                new HashMap<>();

            String hideIdentity =
                crimesRs.getString("HIDE_IDENTITY");

            crime.put(
                "crimeId",
                crimesRs.getInt("CRIME_ID")
            );

            crime.put(
                "category",
                crimesRs.getString("CATEGORY")
            );

            crime.put(
                "description",
                crimesRs.getString("DESCRIPTION")
            );

            crime.put(
                "status",
                crimesRs.getString("STATUS")
            );

            Timestamp ts =
                crimesRs.getTimestamp("DATE_OF_INCIDENT");
            ...
            byte[] demoBytes =
                crimesRs.getBytes("DEMO_PICTURE");

            crime.put(
                "demoImg",
                (demoBytes != null)
                    ? Base64.getEncoder()
                        .encodeToString(demoBytes)
                    : ""
            );
            ...
            if(userRs.next()){
                ...
                byte[] profileBytes =
                    userRs.getBytes("PROFILE_PICTURE");

                if(profileBytes != null){

                    profileImg =
                        Base64.getEncoder()
                        .encodeToString(profileBytes);
                }

                mobileNo =
                    userRs.getString("MOBILE");

                fullNameReal =
                    userRs.getString("FULL_NAME");

                userNameReal =
                    userRs.getString("USER_NAME");
            }
...
body{
    ...
    background:url("images/adminMan.png")
...
<div class="navbar">
    ...
    <div class="user-info">
        ...
        <% if(imageBytes != null){ %>
            ...
            <img
                class="user-pic"
                src="data:image/jpeg;base64,<%= Base64.getEncoder().encodeToString(imageBytes) %>"
            />
...
<div class="content-box">
    ...
    <div class="crime-container">
        ...
        <% if(!((String)crime.get("demoImg")).isEmpty()){ %>
            ...
            <img
                src="data:image/jpeg;base64,<%= crime.get("demoImg") %>"
                class="crime-image"
            />
        ...
        <div class="status-buttons">
            ...
            <form method="post">
                ...
                <% if("ACCEPTED".equalsIgnoreCase((String)crime.get("status"))) { %>
    ...
    <form method="post" onsubmit="return confirm('Delete this report?');" style="margin-top:10px;">
...
<% } else { %>
    ...
    <form method="post">
        ...
        <button type="submit" class="btn allow-btn">
            Allow Post
        </button>
    </form>
